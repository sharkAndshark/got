...
func readSources() {
	...
	filepath.Walk(recipeDir, func(path string, info fs.FileInfo, err error) error {
		...
		if err != nil {
			log.Printf("open and read %s failed, skipped", absName)
		}
		tempPG := pg{}
		...
		cleanName := strings.TrimSuffix(info.Name(), ext)
		SOURCES[cleanName] = source(tempPG)
		log.Printf("found source \"%s\": id: %s minzoom: %d maxzoom: %d", cleanName, cleanName, tempPG.MinZoom, tempPG.MaxZoom)
		return nil
	})
	return
}
